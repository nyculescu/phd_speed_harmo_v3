...
class MARVELReward(RewardFunction):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.congestion_threshold = 35 / 3.6  # 35 mph in m/s
        self.max_step_down = 10  # mph
        self.downstream_action = 70  # Default
    ...
    def calculate(self, metrics: TrafficMetrics,
        ...
        # Get current action (speed limit) in mph
        current_action = metrics.current_speed_limit
        
        # 1. Adaptability reward
        r1 = 0.0
        if metrics.avg_speed_before <= self.congestion_threshold and current_action != 30:
            r1 = -10.0
        ...
        if hasattr(self, 'agent_index') and self.agent_index == 0:
            ...
            r2 = 0.0
        elif self.downstream_action == 30 and current_action in [30, 40]:
            ...
            # Valid transitions
            if (self.downstream_action == 40 and current_action == 50) or \
               (self.downstream_action == 50 and current_action == 60) or \
               (self.downstream_action in [60, 70] and current_action == 70):
                r2 = 2.0
                
        # 3. Mobility reward
        next_speed = metrics.avg_speed_before  # Using current as proxy for next
        v_max = 70 / 3.6  # 70 mph in m/s
