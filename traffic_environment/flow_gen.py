...
def flow_generation(model, idx, daily_pattern, num_days):
    ...
    with open(f"./traffic_environment/sumo/generated_flows_{model}_{idx}.rou.xml", "w") as f:
                    ...
                    if vehs_gen > 0:
                        if "disobedient" in vehicle_type and addDisobedientVehicles:
                            flows.append((day_index, begin_time,
                                        f'    <flow id="{vehicle_type}_flow_{i}_day_{day_index}_halfhr_0" type="{vehicle_type}" begin="{begin_time}" end="{end_time}" '
        ...
        emissionClass = "HBEFA4/PC_petrol_Euro-4"
        f.write(f'    <vType id="disobedient_passenger" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{200/3.6}" color="1,0,0" personNumber="{(int)(np.random.uniform(1, 5))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(3.0, 3.4)
        decel = np.random.uniform(4.5, 5.0)
        length = np.random.triangular(3.7, 4.4, 5.0) * np.random.uniform(0.98, 1.02)
        ...
        emissionClass = "HBEFA4/PC_petrol_Euro-5"
        f.write(f'    <vType id="disobedient_passenger/hatchback" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{240/3.6}" color="0.8,0.8,0" personNumber="{(int)(np.random.uniform(1, 3))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(1.5, 2.0)
        decel = np.random.uniform(3.0, 3.4)
        length = np.random.triangular(3.7, 4.5, 5.0) * np.random.uniform(0.98, 1.02)
        impatience = np.random.uniform(0.5, 1)
        sigma=np.random.uniform(0.5, 1)
        emissionClass = "HBEFA4/PC_petrol_Euro-5"
        f.write(f'    <vType id="disobedient_passenger/van" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{130/3.6}" color="1,1,1" personNumber="{(int)(np.random.uniform(1, 12))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(1.0, 1.5)
        decel = np.random.uniform(3.5, 4.1)
        length = np.random.triangular(12.0, 12.5, 18.0) * np.random.uniform(0.98, 1.02)
        impatience = np.random.uniform(0.5, 1)
        sigma=np.random.uniform(0.5, 1)
        emissionClass = "HBEFA4/RT_le7.5t_Euro-VI_A-C"
        f.write(f'    <vType id="disobedient_bus" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{100/3.6}" color="1,.5,.5" personNumber="{(int)(np.random.uniform(1, 40))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(3.5, 4.1)
        decel = np.random.uniform(5.0, 6.4)
        length = np.random.uniform(2.2, 2.6)
        impatience = np.random.uniform(0.75, 1)
        ...
        f.write(f'    <vType id="disobedient_motorcycle" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{180/3.6}" color=".2,.2,.8" personNumber="{(int)(np.random.uniform(1, 2))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(1.4, 1.8)
        decel = np.random.uniform(3.3, 3.8)
        impatience = np.random.uniform(0.5, 1)
        sigma=np.random.uniform(0.5, 1)
        emissionClass = "HBEFA4/RT_le7.5t_Euro-VI_A-C"
        f.write(f'    <vType id="disobedient_truck" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="6" maxSpeed="{100/3.6}" color=".4,.1,.4" personNumber="{(int)(np.random.uniform(1, 3))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(1.0, 1.4)
        decel = np.random.uniform(3.0, 3.4)
        impatience = np.random.uniform(0.5, 1)
        sigma=np.random.uniform(0.5, 1)
        emissionClass = "HBEFA4/RT_le7.5t_Euro-VI_A-C"
        f.write(f'    <vType id="disobedient_truck/trailer" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="10.5" maxSpeed="{100/3.6}" color=".4,.4,.4" personNumber="{(int)(np.random.uniform(1, 2))}" sigma="{sigma}" impatience="{impatience}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        ...
        decel = np.random.uniform(4.3, 4.7)
        length = np.random.triangular(3.7, 4.75, 5.7) * np.random.uniform(0.97, 1.07)
        f.write(f'    <vType id="electric_passenger" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{200/3.6}" color="1,0,0" personNumber="{(int)(np.random.uniform(1, 5))}" sigma="{np.random.uniform(0.1, 0.5)}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(3.0, 3.4)
        decel = np.random.uniform(4.5, 5.0)
        length = np.random.triangular(3.7, 4.4, 5.0) * np.random.uniform(0.98, 1.02)
        f.write(f'    <vType id="electric_passenger/hatchback" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{240/3.6}" color="0.8,0.8,0" personNumber="{(int)(np.random.uniform(1, 3))}" sigma="{np.random.uniform(0.1, 0.8)}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(1.5, 2.0)
        decel = np.random.uniform(3.0, 3.4)
        length = np.random.triangular(3.7, 4.5, 5.0) * np.random.uniform(0.98, 1.02)
        f.write(f'    <vType id="electric_passenger/van" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{130/3.6}" color="1,1,1" personNumber="{(int)(np.random.uniform(1, 12))}" sigma="{np.random.uniform(0.1, 0.5)}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(1.0, 1.5)
        decel = np.random.uniform(3.5, 4.1)
        length = np.random.triangular(12.0, 12.5, 18.0) * np.random.uniform(0.98, 1.02)
        f.write(f'    <vType id="electric_bus" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="{length}" maxSpeed="{100/3.6}" color="1,.5,.5" personNumber="{(int)(np.random.uniform(1, 40))}" sigma="{np.random.uniform(0.1, 0.3)}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
        accel = np.random.uniform(3.5, 4.1)
        decel = np.random.uniform(5.0, 6.4)
        length = np.random.uniform(2.2, 2.6)
        ...
        accel = np.random.uniform(1.0, 1.4)
        decel = np.random.uniform(3.0, 3.4)
        f.write(f'    <vType id="electric_truck/trailer" carFollowModel= "{car_following_model}" accel="{accel}" decel="{decel}" length="10.5" maxSpeed="{100/3.6}" color=".4,.4,.4" personNumber="{(int)(np.random.uniform(1, 2))}" sigma="{np.random.uniform(0.1, 0.3)}" speedFactor="1.2" speedDev="0.2" lcStrategic="0.5" lcCooperative="0.1" lcSpeedGain="1.5" emissionClass="{emissionClass}"/>\n')
