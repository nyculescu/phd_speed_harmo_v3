...
def determine_speed_limit(occupancy, avg_speed):
    """
    Determines appropriate speed limit based on traffic conditions
    Returns speed in km/h from [50, 55,..., 125, 130]
    """
    if occupancy > 80:
        return 50  # Severe congestion
    elif 40 < occupancy <= 80:
        return min(max(round(avg_speed/5)*5, 50), 80)  # Moderate congestion
    elif 12 < occupancy <= 40:
        return min(max(round(avg_speed/5)*5, 50), 100)  # Light congestion
    else:
        return 130  # Free flow
